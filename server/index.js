  ...
  socket.on('disconnect', () => {
    ...
    try {
      if (currentRoom && rooms[currentRoom]) {
        ...
        if (rooms[currentRoom].players[rooms[currentRoom].currentPlayerIndex]?.id === socket.id) {
          const validMoves = rooms[currentRoom].getValidMoves(socket.id);
...
server.listen(PORT, () => {
  ...
  console.log(`🌐 Or share your IP address with family: http://YOUR_IP:${PORT}`);
});