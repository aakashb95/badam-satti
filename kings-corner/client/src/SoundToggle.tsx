    ...
    <div className="sound-control" ref={controlRef}>
      ...
      {isOpen && (
        <div className="sound-menu" id={menuId} role="group" aria-label="Sound settings">
          <button className="sound-menu-option" role="checkbox" aria-checked={backgroundMusicOn} onClick={() => onBackgroundMusicChange(!backgroundMusicOn)}>
            <span className={`sound-menu-check ${backgroundMusicOn ? 'is-checked' : ''}`} aria-hidden="true">{backgroundMusicOn ? '✓' : ''}</span>
            <span>Background music</span>
          </button>
          <label className={`sound-volume ${backgroundMusicOn ? '' : 'is-disabled'}`}>
            <span>Music volume <output>{Math.round(backgroundMusicVolume * 100)}%</output></span>
            <input
              type="range"
              min="0"
              max="100"
          ...
          <button className="sound-menu-option" role="checkbox" aria-checked={gameSoundsOn} onClick={() => onGameSoundsChange(!gameSoundsOn)}>
            <span className={`sound-menu-check ${gameSoundsOn ? 'is-checked' : ''}`} aria-hidden="true">{gameSoundsOn ? '✓' : ''}</span>
