...
export default function CardView({ card, className = '', onClick, label }: Props) {
  ...
  const image = <><span className={`card-fallback ${card.suit === 'hearts' || card.suit === 'diamonds' ? 'red' : ''}`} aria-hidden="true"><strong>{rankCode(card.rank)}</strong><i>{suitSymbol[card.suit]}</i></span><img className={loaded ? 'loaded' : ''} src={`/images/cards/${rankCode(card.rank)}${suitCode[card.suit]}.svg`} alt={name} draggable={false} onLoad={() => setLoaded(true)} /></>;
