...
const LESSONS: Array<{ section: 'Game rules' | 'Using the table'; eyebrow: string; title: string; copy: string; visual: Visual }> = [
  ...
  { section: 'Game rules', eyebrow: 'Build the piles', title: 'Go down. Alternate colours.', copy: 'A black Jack fits on a red Queen. A red Ten fits on that black Jack.', visual: 'build' },
...
function VisualStage({ visual }: { visual: Visual }) {
  return <div className={`kc-lesson-stage kc-${visual}`} aria-hidden="true">
    ...
    {visual === 'king' && <><span className="kc-corner-slot">K</span><div className="kc-stock">K</div><Card rank="K" suit="♠" className="kc-moving-king" /><Card rank="5" suit="♥" className="kc-replacement" /><span className="kc-caption">King → corner → draw again</span></>}
